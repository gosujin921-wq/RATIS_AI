...
export interface Evidence {
  ...
  caption?: string
  /**
   * 원문 파일명. 원문 패널의 제목이 이 값이다 — 사용자가 내려받은 파일과
   * 화면에서 본 문서가 같은 것임을 이름으로 확인한다.
   * ⚠ **API 계약에는 아직 없다.** 계약이 확정되면 필드명을 거기에 맞춘다.
   */
  fileName?: string
  /**
   * 원문 파일을 통째로 내려받는 자리. 비어 오면 화면은 다운로드 버튼을 그리지 않는다.
   * ⚠ 실제 파일 제공 방식(직접 다운로드·서명 URL·권한 검사)은 개발 협의 항목이다.
   */
  fileUrl?: string
  /** 원문 전체 쪽수. 원문 패널의 「n / m쪽」이 읽는다. 없으면 쪽 이동을 열지 않는다 */
  pageCount?: number
}
...
export interface ChatMessage {
  ...
  evidences: Evidence[]
  /**
   * 답변을 정리한 보고서. 있으면 답변 아래에 받는 링크 한 줄이 선다.
   * ⚠ **파일로 내려받을지 링크로 열지는 미확정이다** — 지금은 링크로 두고, 방식이
   *   정해지면 화면을 다시 잡는다 (2026-09-03 검토).
   */
